...
### User Story 7 - 앱별 스플릿 터널링 (Priority: P3)

특정 앱의 트래픽만 VPN으로 보내거나, 특정 앱을 VPN에서 제외한다.

**Acceptance Scenarios**:

1. **Given** 스플릿 터널링 설정, **When** "Chrome만 VPN 통과"를 설정하면, **Then** Chrome 트래픽만 VPN을 통하고 나머지는 직접 나간다.

---

### User Story 8 - 미니 모드 (Priority: P3)
...
### User Story 9 - 네트워크 진단 + 속도 테스트 (Priority: P3)
...
### User Story 10 - 키보드 단축키 (Priority: P3)
...
### User Story 11 - 동시 멀티 터널 (Priority: P3)

여러 터널을 동시에 연결하되, 라우팅 충돌을 자동 관리한다.
...
### User Story 12 - DNS 누출 테스트 + 라우트 시각화 (Priority: P3)
...
- 앱별 스플릿이 지원 안 되는 OS? → 해당 기능 비활성화 + 안내 메시지
...
- **FR-007**: 앱별 스플릿 터널링 (OS 지원 범위에서)
- **FR-008**: 미니 모드 플로팅 위젯
- **FR-009**: CIDR 계산기 + 엔드포인트 도달 테스트 + 속도 테스트
- **FR-010**: 키보드 단축키 (Cmd+1~9 터널, 글로벌 VPN 토글)
- **FR-011**: 동시 멀티 터널 + 라우팅 충돌 관리
